...
function init() {
    ...
    sightings = loadData(function(data) { return data; });
   // }, 1500);
}
...
function loadData(fn) {
    d3.json(dataConfig.data_url, refreshData);
};

// Asyncronous queue to refresh table data, and show progress bar
function refreshData(data) {
    var q = d3.queue();
    q.defer(buildProgressBar, data);
    q.defer(renderTable, data); 
...
// Table generation
function renderTable(filteredSightings, cb) {
    ...
    cb(null, filteredSightings);
}

// Progress bar function
function buildProgressBar(data, cb) {
